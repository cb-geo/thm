...
// Pressure seetings
const int g_num_P_bnd_id = 2;  // numbers of  pressure boudnary condition id
const int g_P_bnd_id[g_num_P_bnd_id] = {
    1, 99999};  // pressure boundary condition id

const double g_Pb_top = 0;        // pressure at the top of model
const double g_Pb_well = 100000;  // pressure at the top of model
const double g_P_grad = 0;        // pressure gradient in vertial direction
const double g_P_grad_x = 0;      // 2000m drops
// ten meters
...
const int g_T_bnd_id[g_num_T_bnd_id] = {1, 99999};
...
const double g_T_grad = 0.0;  // temperature gradient in verital direction
...
const double g_Tb_well = 273.15 + 30;  // average pile temperature

// Heat flow rate settinsy
...
const double g_total_time = 1 * 360;  // simulation time
const double g_period = 1 * 360;      // for periodic load
const int g_n_time_step = 6;          // simulation time
...
std::vector<double> g_time_sequence = {0, 10, 20, 50, 100, 360};
